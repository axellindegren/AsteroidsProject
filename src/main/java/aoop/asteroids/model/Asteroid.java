...
public class Asteroid extends GameObject implements Serializable{
	 ...
	 */
	public Collection <GameObject> getSuccessors () {
	...
	public void draw(Graphics2D g) {
		...
		e.setFrame (getLocation ().x - getRadius (), getLocation ().y - getRadius (), 2 * getRadius (), 2 * getRadius ());
		g.fill(e);
	}

	@Override
	public void onRendererAttached(Renderer parent) { /*We don't do anything with it */}

	@Override
    public void destroy(DestroyReason destroyReason) {
        if (destroyReason instanceof CollisionWithObject) {
            if (!(((CollisionWithObject) destroyReason).getGameObject() instanceof Asteroid)) super.destroy(destroyReason);
        }
    }
}
